...
if (win.electronAPI) {
  ...
  const redirectToLogin = () => {
    ...
    // 短暂延迟以便展示 UI 反馈；PWA 通过顶层请求重新经过 Cloudflare，
    // 一般 WebUI 则留在 HashRouter 内，避免落到空 hash 造成白屏。
    // Surface UI feedback briefly; PWA reloads through Cloudflare while the
    // regular WebUI stays inside HashRouter instead of landing on an empty hash.
    setTimeout(() => {
      if (isPwaStandalone) {
